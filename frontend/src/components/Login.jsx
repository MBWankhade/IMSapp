    ...
    const handleLogin = async () => {
        try {
            ...
            if (res.ok) { // Check if the response is successful
                ...
                localStorage.setItem("token", data.data); // Store the token in local storage
    ...
    return (
        ...
        <div className='flex flex-col shadow-xl border border-gray-300 p-12 rounded-xl w-3/12 justify-center gap-10 items-center'>
            ...
            <input
                ...
                className='p-2 w-10/12 text-2xl font-semibold border-solid border-2 border-gray-500 text-black rounded-md my-2 outline-none'
            />
            <button onClick={handleLogin} className='bg-blue-400 w-10/12 text-2xl py-1 px-2 font-semibold text-white rounded-md w-fit'>Login</button>
            <p className='text-xl font-light '>New User? <Link className='text-blue-400 font-semibold' to="/signup">Sign Up</Link></p>
        </div>
        </div>
        </>
    );
